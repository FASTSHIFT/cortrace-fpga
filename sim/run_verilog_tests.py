...
try:
    ...
    print(
        "error: PyYAML required (apt install python3-yaml OR pip install pyyaml)",
        file=sys.stderr,
    )
    sys.exit(2)
...
def run_one(tc: TestCase, workdir: Path, keep: bool) -> TestResult:
    ...
    try:
        r = subprocess.run(
            ivl_cmd, capture_output=True, text=True, timeout=tc.timeout_s
        )
        log += r.stdout + r.stderr
        if r.returncode != 0:
            return TestResult(
                tc, False, f"iverilog exit {r.returncode}", time.time() - t0, log
            )
    ...
    try:
        r = subprocess.run(
            ["vvp", str(vvp_out)], capture_output=True, text=True, timeout=tc.timeout_s
        )
    except subprocess.TimeoutExpired:
    ...
    for pat in tc.expect_contains:
        if pat not in out:
            return TestResult(
                tc, False, f"missing marker: {pat!r}", time.time() - t0, log
            )
    for pat in tc.expect_regex:
        if not re.search(pat, out):
            return TestResult(
                tc, False, f"no regex match: {pat!r}", time.time() - t0, log
            )
    for pat in tc.expect_not_contains:
        if pat in out:
            return TestResult(
                tc, False, f"forbidden marker present: {pat!r}", time.time() - t0, log
            )
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        action="append",
        default=[],
        help="run only tests whose full name matches this glob " "(may repeat)",
    )
    ap.add_argument("--keep", action="store_true", help="keep .vvp files after run")
    ap.add_argument("--verbose", action="store_true", help="print full log on failure")
    ap.add_argument("--list", action="store_true", help="list matching tests and exit")
    ap.add_argument(
        "--workdir", default=None, help="working directory for build artefacts"
    )
    ap.add_argument("--junit", default=None, help="write JUnit XML report here")
    ap.add_argument(
        "--coverage",
        action="store_true",
    ...
    workdir_ctx = (
        tempfile.TemporaryDirectory() if not args.workdir else _NopCtx(args.workdir)
    )
    with workdir_ctx as wd:
        ...
        for tc in selected:
            ...
            print(
                f"  {_color(tag[0], tag[1])} {elapsed}  {tc.full_name()}"
        ...
        if args.coverage:
            ...
            for tc in tests:  # all tests, not just selected
                for src in tc.sources:
                    ...
                    if (
                        "/sim/" in lower
                        or "/testbeds/" in lower
                        or "/tb_" in lower
                        or lower.endswith("_tb.v")
                    ):
                        continue
            ...
            print(
                "  (NOTE: this is file-level presence, not line-coverage — "
        ...
        summary = (
            f"Summary: {n_pass} passed, {n_fail} failed in {fmt_elapsed(elapsed_total)}"
        )
        if n_fail == 0:
            ...
            if not args.verbose:
                for r in results:
                    if not r.passed:
                        print(
                            f"  FAILED: {r.tc.full_name()} - {r.reason}"
...
def write_junit(results: list[TestResult], total_s: float, path: Path) -> None:
    ...
    lines.append(
        f'<testsuite name="verilog" tests="{n}" failures="{n_fail}" '
        f'time="{total_s:.3f}">'
    )
    for r in results:
        cls = r.tc.group.replace("/", ".")
        lines.append(
            f'  <testcase classname="{escape(cls)}" '
        ...
        if not r.passed:
            log = escape(r.log or "")
            lines.append(
                f'    <failure message="{escape(r.reason)}">'
                f"<![CDATA[\n{log}\n]]></failure>"
            )
